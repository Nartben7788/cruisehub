...
def reservation(car_id):
    ...
    if request.method == 'GET':
        ...
        if 'user_id' not in session:    
            ...
            user_msg.body=f' Dear {user.name}. This is confirming your reservation with the reservation ID : {reservation.id} from {start_date} to {end_date}'
            mail.send(user_msg)


            owner_msg = Message('New Reservation' ,sender= 'cruise.carhub@gmail.com', recipients= [owner.email])
            user_msg.body=f' Dear {owner.name}. Your car {car.make} {car.model} ID : {car.id} has been reserved by {user.name} with the reservation 
            ID : {reservation.id} from {start_date} to {end_date}'
            mail.send(owner_msg)
            db.session.commit()
